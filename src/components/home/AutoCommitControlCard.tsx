    ...
    <div className="bg-white border border-neutral-200/90 rounded-2xl p-5 sm:p-6 shadow-xs space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-bold text-neutral-950">GitHub Auto-Commit Control</h3>
              {autoCommitEnabled ? (
      ...
      {uncommittedCount > 0 ? (
        <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-4 space-y-3.5">
          ...
          {showFilesList && (
            <div className="bg-white/90 border border-amber-200/80 rounded-lg p-2.5 max-h-48 overflow-y-auto space-y-1.5">
              {uncommittedFiles.map((file, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between gap-2 text-xs p-1.5 rounded-md hover:bg-neutral-50 border border-neutral-100 font-mono"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <FileCode className="w-3.5 h-3.5 text-neutral-400 shrink-0" />
                    <span className="truncate text-neutral-800 text-[11px]">{file.path}</span>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1">
            <input
              ...
              className="flex-1 px-3 py-2 bg-white border border-amber-300/80 rounded-xl text-xs text-neutral-800 placeholder-neutral-400 focus:outline-hidden focus:ring-2 focus:ring-amber-500/20"
            />
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-neutral-900 hover:bg-neutral-800 active:bg-neutral-950 text-white text-xs font-semibold flex items-center justify-center gap-1.5 shadow-xs transition-colors disabled:opacity-50"
            >
              {isCommitting ? (
                <>
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl border border-neutral-300 hover:bg-neutral-100 text-neutral-700 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors disabled:opacity-50"
            >
              {isDiscarding ? (
                <Loader2 className="w-3.5 h-3.5 animate-spin" />
              ) : (
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-neutral-50/80 border border-neutral-200/70 text-xs">
