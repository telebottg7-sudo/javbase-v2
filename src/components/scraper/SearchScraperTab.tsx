    ...
    <div className="space-y-6">
      ...
      <div className="p-5 bg-white dark:bg-[#101728] border border-neutral-200 dark:border-[#1e293b] rounded-2xl shadow-xs space-y-4">
        ...
        <form onSubmit={onSearch} className="space-y-3">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-neutral-50 dark:bg-[#0b101a] border border-neutral-200 dark:border-[#1e293b] rounded-xl text-xs focus:outline-hidden focus:ring-2 focus:ring-neutral-900/10 focus:border-neutral-900 transition-all"
              />
            </div>
          ...
          <div className="flex items-center gap-4 text-xs text-neutral-600 dark:text-slate-400 pt-1 flex-wrap">
            <label className="flex items-center gap-2 cursor-pointer select-none">
              <input
                type="checkbox"
                checked={filterDuplicates}
                onChange={(e) => setFilterDuplicates(e.target.checked)}
                className="rounded border-neutral-300 dark:border-[#2b3a54] text-neutral-900 dark:text-white focus:ring-neutral-900"
              />
      ...
      {searchResult && (
        <div className="space-y-4">
          <div className="p-4 bg-white dark:bg-[#101728] border border-neutral-200 dark:border-[#1e293b] rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs text-neutral-600 dark:text-slate-400 shadow-xs">
            <div className="flex items-center gap-2 flex-wrap">
              <span>
                Found <strong className="text-neutral-900 dark:text-white">{searchResult.totalFound}</strong> results
                for query &ldquo;<strong className="text-neutral-900 dark:text-white">{searchQuery}</strong>&rdquo;
                {searchPage > 1 && (
                  <span className="text-neutral-500 dark:text-slate-400 font-medium"> (Page {searchPage})</span>
                )}
              </span>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {searchResult.source && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="font-mono text-[11px] text-neutral-500 dark:text-slate-400 hover:text-neutral-900 dark:text-white flex items-center gap-1 transition-colors px-2.5 py-1.5 bg-neutral-50 dark:bg-[#0b101a] border border-neutral-200 dark:border-[#1e293b] rounded-lg"
          ...
          {searchResult.items.length === 0 ? (
            <div className="p-12 text-center bg-white dark:bg-[#101728] border border-neutral-200 dark:border-[#1e293b] rounded-2xl text-xs text-neutral-500 dark:text-slate-400">
